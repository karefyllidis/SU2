...
public:
  ...
  */
   ...
   */
  inline su2double GetCoordCornerPoints(unsigned short val_dim, unsigned short val_icornerpoints) const { return Coord_Corner_Points[val_icornerpoints][val_dim]; }
   ...
   */
  inline su2double *GetCoordCornerPoints(unsigned short val_icornerpoints) const { return Coord_Corner_Points[val_icornerpoints]; }
   ...
   */
  inline su2double *GetCoordControlPoints(unsigned short val_iindex, unsigned short val_jindex, unsigned short val_kindex) const { return Coord_Control_Points[val_iindex][val_jindex][val_kindex]; }
   ...
   * \return Pointer to the coordinate vector of the control point with local index (i, j, k).
   */
  inline su2double *GetParCoordControlPoints(unsigned short val_iindex, unsigned short val_jindex, unsigned short val_kindex) const { return ParCoord_Control_Points[val_iindex][val_jindex][val_kindex]; }
   ...
   */
  inline su2double DotProduct(const su2double *v1, const su2double *v2) { su2double scalar = v1[0]*v2[0]+v1[1]*v2[1]+v1[2]*v2[2]; return scalar; }
   ...
   * \return Order in the l direction of the FFD FFDBox.
   */
  inline void SetlOrder(unsigned short val_lOrder) { lOrder = val_lOrder; lDegree = lOrder-1; }

  /*!
   * \brief Get the order in the m direction of the FFD FFDBox.
   * \return Order in the m direction of the FFD FFDBox.
   */
  inline void SetmOrder(unsigned short val_mOrder) { mOrder = val_mOrder; mDegree = mOrder-1; }

  /*!
   * \brief Get the order in the n direction of the FFD FFDBox.
   * \return Order in the n direction of the FFD FFDBox.
   */
  inline void SetnOrder(unsigned short val_nOrder) { nOrder = val_nOrder; nDegree = nOrder-1;}
   ...
   * \return __________.
   */
  inline su2double GetNorm(const su2double *a) { su2double  norm = sqrt(a[0]*a[0] + a[1]*a[1]+ a[2]*a[2]); return norm; }
